...
import utils
import ggnet_generator as generator
from losses import GGNetLoss
from Models.gg_net import GGNet
...
def main(FLAGS):
    ...
    for val_round_index in range(num_validation_rounds):
        ...
        loss_fn = GGNetLoss(img_size=image_size)
        ...
        @tf.function
        def train_step(img, label, bd_label):
            with tf.GradientTape() as tape:
                out = model(img, training=True)
                loss_fn.set_bd_label(bd_label)
        ...
        for epoch in tqdm.tqdm(range(epochs)):
            ...
            for batch_index in range(epoch_size):
                if epoch == 3 and 10 <= batch_index <= 15:
                    with tf.profiler.experimental.Trace('train', step_num=optimizer_step, _r=1):
                        img, label, bd_label = training_generator[batch_index]
                        out, loss = train_step(img, label, bd_label)
                else:
                    img, label, bd_label = training_generator[batch_index]
                    out, loss = train_step(img, label, bd_label)
            ...
            with tf.device("cpu:0"):  # Prevent OOM
                ...
                for val_batch_index in range(n_val_batches):
                    ...
                    val_segmentation_batch_bd = (val_segmentation_batch * 255).astype(np.uint8)
                    val_segmentation_batch_bd = cv2.Canny(val_segmentation_batch_bd, 100, 200)
                    val_segmentation_batch_bd = val_segmentation_batch_bd / 255
                    val_predictions = model(val_ultrasound_batch)
                    loss_fn.set_bd_label(val_segmentation_batch_bd)
                    epoch_val_loss += loss_fn(val_segmentation_batch, val_predictions) / n_val_batches
                    metric.update_state(val_segmentation_batch, val_predictions[0])
